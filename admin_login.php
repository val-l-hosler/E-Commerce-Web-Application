...
<main class="login">
    <form action="admin_login.php" method="post">
        ...
        if (isset($final_username, $final_password)) {
            $final_username = addslashes($final_username);
            $final_password = addslashes($final_password);

            $query = "SELECT password FROM admin WHERE username = '$final_username' AND password = '$final_password' LIMIT 1";
